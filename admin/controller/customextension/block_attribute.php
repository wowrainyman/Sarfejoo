...
class ControllerCustomextensionBlockAttribute extends Controller
{
    private $error = array();

    public function index()
    {
        $this->language->load('customextension/block_attribute');
        $this->document->setTitle($this->language->get('heading_title'));
        $this->load->model('customextension/pu_block_attribute');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            foreach ($_POST['subattribute_name'] as $key => $value) {
                $this->model_customextension_pu_block_attribute->updateSubattributeName($key, $value);
            }
    ...
    public function insert()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if (isset($this->request->get['filter_subattribute_name'])) {
                $url .= '&filter_subattribute_name=' . urlencode(html_entity_decode($this->request->get['filter_subattribute_name'], ENT_QUOTES, 'UTF-8'));
            }
            if (isset($this->request->get['filter_block_id'])) {
                $url .= '&filter_block_id=' . urlencode(html_entity_decode($this->request->get['filter_block_id'], ENT_QUOTES, 'UTF-8'));
            }
            ...
            $this->redirect($this->url->link('customextension/block_attribute', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
        $this->getForm();
    }
    ...
    {
        if (isset($this->request->get['filter_subattribute_name'])) {
            $filter_subattribute_name = $this->request->get['filter_subattribute_name'];
        } else {
            $filter_subattribute_name = null;
        }
        if (isset($this->request->get['filter_block_id'])) {
            $filter_block_id = $this->request->get['filter_block_id'];
        } else {
            $filter_block_id = null;
        }
        if (isset($this->request->get['filter_type'])) {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('customextension/block_attribute', 'token=' . $this->session->data['token'] . $url, 'SSL'),
            'separator' => ' :: '
        );
        $this->data['insert'] = $this->url->link('customextension/block_attribute/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $this->data['products'] = array();
        $data = array(
            'filter_subattribute_name' => $filter_subattribute_name,
        ...
        $block_total = $this->model_customextension_pu_block_attribute->getTotalBlockAttributes($data);
        $results = $this->model_customextension_pu_block_attribute->getBlockAttributes($data);
        $this->data['form_action'] = $this->url->link('customextension/block_attribute', 'token=' . $this->session->data['token'] . $url, 'SSL');
        foreach ($results as $result) {
            $action = array();
            $action[] = array(
                'text' => $this->language->get('text_edit'),
                'href' => $this->url->link('customextension/block_attribute/update', 'token=' . $this->session->data['token'] . '&block_id=' . $result['id'] . $url, 'SSL')
            );
            $this->load->model('customextension/pu_block');
            $block_info = $this->model_customextension_pu_block->getBlock($result['block_id']);
            $this->data['blockAttributes'][] = array(
                'id' => $result['id'],
        ...
        $this->data['column_block_id'] = $this->language->get('column_block_id');
        $this->data['column_subattribute_name'] = $this->language->get('column_subattribute_name');
        ...
        $this->data['sort_subattribute_name'] = $this->url->link('customextension/block_attribute', 'token=' . $this->session->data['token'] . '&sort=tab1.subattribute_name' . $url, 'SSL');
        $this->data['sort_block_id'] = $this->url->link('customextension/block_attribute', 'token=' . $this->session->data['token'] . '&sort=tab1.block_id' . $url, 'SSL');
        $this->data['sort_type'] = $this->url->link('customextension/block_attribute', 'token=' . $this->session->data['token'] . '&sort=tab1.type' . $url, 'SSL');
        $url = '';
        ...
        $pagination->url = $this->url->link('customextension/block_attribute', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

        $this->data['pagination'] = $pagination->render();

        $this->data['filter_subattribute_name'] = $filter_subattribute_name;
        $this->data['filter_block_id'] = $filter_block_id;
        $this->data['filter_type'] = $filter_type;

        $this->data['sort'] = $sort;
        $this->data['order'] = $order;

        $this->template = 'customextension/block_attribute_list.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    protected function getForm()
    {
        ...
        if (!isset($this->request->get['product_id'])) {
            $this->data['action'] = $this->url->link('customextension/block_attribute/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $this->data['action'] = $this->url->link('customextension/block_attribute/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
        }
        ...
        $this->template = 'customextension/block_attribute_form.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    public function autocomplete() {
        ...
        if (isset($this->request->get['filter_name'])) {
            ...
            $data = array(
                'filter_block_name' => $this->request->get['filter_name'],
                'start'       => 0,
                'limit'       => 20
            );
            ...
            $results = $this->model_customextension_pu_block->getBlocks($data);

            foreach ($results as $result) {
                $json[] = array(
                    'id'    => $result['id'],
                    'name'            => strip_tags(html_entity_decode($result['block_name'], ENT_QUOTES, 'UTF-8'))
                );
            }
        }
