...
class ControllerCustomextensionBlockAttributeValue extends Controller
{
    private $error = array();

    public function index()
    {
        $this->language->load('customextension/block_attribute_value');
        $this->document->setTitle($this->language->get('heading_title'));
        $this->load->model('customextension/pu_block_attribute_value');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            foreach ($_POST['value'] as $key => $value) {
                $this->model_customextension_pu_block_attribute_value->updateValue($key, $value);
            }
        }
        $this->getList();
    }
    public function insert()
    {
        ...
        $this->load->model('customextension/pu_block_attribute_value');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            $block_attribute_id = $this->request->post['block_attribute_id'];
            foreach ($_POST['value'] as $key => $value) {
                if($value!='') {
                    $this->model_customextension_pu_block_attribute_value->addBlockAttributeValue($block_attribute_id, $value);
                }
            }
            ...
            $this->redirect($this->url->link('customextension/block_attribute_value', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
        $this->getForm();
    }
    protected function getList()
    {
        ...
        $this->data['insert'] = $this->url->link('customextension/block_attribute_value/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $block_total = $this->model_customextension_pu_block_attribute_value->getTotalBlockAttributeValues($data);
        $results = $this->model_customextension_pu_block_attribute_value->getBlockAttributeValues($data);
        $this->data['form_action'] = $this->url->link('customextension/block_attribute_value', 'token=' . $this->session->data['token'] . $url, 'SSL');
        foreach ($results as $result) {
            $action = array();
            $action[] = array(
                'text' => $this->language->get('text_edit'),
                'href' => $this->url->link('customextension/block_attribute_value/update', 'token=' . $this->session->data['token'] . '&block_id=' . $result['id'] . $url, 'SSL')
            );
            $this->load->model('customextension/pu_block_attribute');
            $block_attribute_info = $this->model_customextension_pu_block_attribute->getBlockAttribute($result['block_attribute_id']);
            $this->data['blockAttributeValues'][] = array(
                'id' => $result['id'],
                'subattribute_name' => $block_attribute_info['subattribute_name'],
                'value' => $result['value'],
                'action' => $action
            );
        }
        ...
        $this->data['sort_value'] = $this->url->link('customextension/block_attribute_value', 'token=' . $this->session->data['token'] . '&sort=tab1.value' . $url, 'SSL');
        $url = '';

        if (isset($this->request->get['filter_valuee'])) {
            $url .= '&filter_valuee=' . urlencode(html_entity_decode($this->request->get['filter_valuee'], ENT_QUOTES, 'UTF-8'));
        }
        ...
        $pagination->url = $this->url->link('customextension/block_attribute_value', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->template = 'customextension/block_attribute_value_list.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    protected function getForm()
    {
        ...
        if (!isset($this->request->get['product_id'])) {
            $this->data['action'] = $this->url->link('customextension/block_attribute_value/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $this->data['action'] = $this->url->link('customextension/block_attribute_value/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
        }
        ...
        $this->template = 'customextension/block_attribute_value_form.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    public function autocomplete() {
        ...
        if (isset($this->request->get['filter_name'])) {
            ...
            $data = array(
                'filter_subattribute_name' => $this->request->get['filter_name'],
                'start'       => 0,
                'limit'       => 20
            );
            ...
            foreach ($results as $result) {
                $json[] = array(
                    'id'    => $result['id'],
                    'name'  => strip_tags(html_entity_decode($result['subattribute_name'], ENT_QUOTES, 'UTF-8'))
                );
            }
        }
