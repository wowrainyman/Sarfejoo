...
class ControllerCustomextensionRating extends Controller
{
    private $error = array();

    public function index()
    {
        $this->language->load('customextension/rating');

        $this->document->setTitle($this->language->get('heading_title'));

        $this->load->model('customextension/pu_rating');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            foreach ($_POST['name'] as $key => $value) {
                $this->model_customextension_pu_rating->UpdateName($key, $value);
            }
            foreach ($_POST['enable'] as $key => $value) {
                $this->model_customextension_pu_rating->UpdateEnable($key, $value);
            }
        }
        $this->getList();
    }
    ...
    public function insert()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $this->model_customextension_pu_rating->addRating(
                $name,
                $enable);
            ...
            $this->redirect($this->url->link('customextension/rating', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
        $this->getForm();
    }
    ...
    protected function getList()
    {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('customextension/rating', 'token=' . $this->session->data['token'] . $url, 'SSL'),
            'separator' => ' :: '
        );

        $this->data['insert'] = $this->url->link('customextension/rating/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $rating_total = $this->model_customextension_pu_rating->getTotalRating($data);

        $results = $this->model_customextension_pu_rating->getRatings($data);

        $this->data['form_action'] = $this->url->link('customextension/rating', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        foreach ($results as $result) {
            ...
            $action[] = array(
                ...
                'href' => $this->url->link('customextension/rating/update', 'token=' . $this->session->data['token'] . '&product_id=' . $url, 'SSL')
            );
        ...
        $this->data['column_enable'] = $this->language->get('column_enable');
        ...
        $this->data['sort_name'] = $this->url->link('customextension/rating', 'token=' . $this->session->data['token'] . '&sort=tab1.name' . $url, 'SSL');
        $this->data['sort_enable'] = $this->url->link('customextension/rating', 'token=' . $this->session->data['token'] . '&sort=tab1.enable' . $url, 'SSL');
        ...
        $pagination->url = $this->url->link('customextension/rating', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->template = 'customextension/rating_list.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    protected function getForm()
    {
        ...
        if (!isset($this->request->get['product_id'])) {
            $this->data['action'] = $this->url->link('customextension/rating/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $this->data['action'] = $this->url->link('customextension/rating/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
        }
        ...
        $this->template = 'customextension/rating_form.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
