...
class ControllerCustomextensionAttributeClass extends Controller
{
    private $error = array();

    public function index()
    {
        $this->language->load('customextension/attribute_class');

        $this->document->setTitle($this->language->get('heading_title'));

        $this->load->model('customextension/pu_attribute_class');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            foreach ($_POST['type'] as $key => $value) {
                $this->model_customextension_pu_attribute_class->UpdateAttributeClassType($key, $value);
            }
            foreach ($_POST['class'] as $key => $value) {
                $this->model_customextension_pu_attribute_class->UpdateAttributeClass($key, $value);
            }
        }
        $this->getList();
    }
    ...
    public function insert()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            $id = $this->request->post['attribute_id'];
            $type = $this->request->post['type'];
            $class = $this->request->post['class'];
            $this->model_customextension_pu_attribute_class->addAttributeClass($id, $type, $class);
            ...
            $this->redirect($this->url->link('customextension/attribute_class', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
    ...
    protected function getList()
    {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('customextension/attribute_class', 'token=' . $this->session->data['token'] . $url, 'SSL'),
            'separator' => ' :: '
        );

        $this->data['insert'] = $this->url->link('customextension/attribute_class/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $attribute_total = $this->model_customextension_pu_attribute_class->getTotalAttributes();

        $results = $this->model_customextension_pu_attribute_class->getAttributes($data);
        ...
        foreach ($results as $result) {
            ...
            $action[] = array(
                ...
                'href' => $this->url->link('customextension/attribute_class/update', 'token=' . $this->session->data['token'] . '&attribute_id=' . $result['attribute_id'] . $url, 'SSL')
            );
        ...
        $this->data['sort_name'] = $this->url->link('customextension/attribute_class', 'token=' . $this->session->data['token'] . '&sort=tab3.name' . $url, 'SSL');
        $this->data['sort_attribute_group'] = $this->url->link('customextension/attribute_class', 'token=' . $this->session->data['token'] . '&sort=attribute_group' . $url, 'SSL');
        $this->data['sort_sort_order'] = $this->url->link('customextension/attribute_class', 'token=' . $this->session->data['token'] . '&sort=tab2.sort_order' . $url, 'SSL');
        ...
        $pagination->url = $this->url->link('customextension/attribute_class', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

        $this->data['form_action'] = $this->url->link('customextension/attribute_class', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $this->template = 'customextension/attribute_class_list.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    protected function getForm()
    {
        ...
        $this->data['text_attribute_class'] = $this->language->get('text_attribute_class');
        ...
        if (!isset($this->request->get['product_id'])) {
            $this->data['action'] = $this->url->link('customextension/attribute_class/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $this->data['action'] = $this->url->link('customextension/attribute_class/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
        }
        ...
        $this->template = 'customextension/attribute_class_form.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
