<?php 
class ControllerCustomextensionBlock extends Controller {
	private $error = array(); 

	public function index() {
		$this->language->load('customextension/block');
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model('customextension/pu_block');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            foreach ($_POST['block_name'] as $key => $value) {
                $this->model_customextension_pu_block->updateBlockName($key, $value);
            }
        }
		$this->getList();
	}
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $block_name=$this->request->post['block_name'];
			$this->model_customextension_pu_block->addBlock($attribute_id,$block_name);
			$this->session->data['success'] = $this->language->get('text_success');
			$url = '';
			if (isset($this->request->get['filter_block_name'])) {
				$url .= '&filter_block_name=' . urlencode(html_entity_decode($this->request->get['filter_block_name'], ENT_QUOTES, 'UTF-8'));
			}
			...
			$this->redirect($this->url->link('customextension/block', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		$this->getForm();
	}
	...
	protected function getList() {
		if (isset($this->request->get['filter_block_name'])) {
			$filter_block_name = $this->request->get['filter_block_name'];
		} else {
			$filter_block_name = null;
		}
		if (isset($this->request->get['filter_attribute_id'])) {
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('customextension/attribute', 'token=' . $this->session->data['token'] . $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert'] = $this->url->link('customextension/block/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['products'] = array();
		$data = array(
			'filter_block_name'     => $filter_block_name,
        ...
        $block_total = $this->model_customextension_pu_block->getTotalBlocks($data);
		$results = $this->model_customextension_pu_block->getBlocks($data);
        $this->data['form_action']=$this->url->link('customextension/block', 'token=' . $this->session->data['token'] . $url, 'SSL');
		foreach ($results as $result) {
			$action = array();
			$action[] = array(
				'text' => $this->language->get('text_edit'),
				'href' => $this->url->link('customextension/block/update', 'token=' . $this->session->data['token'] . '&block_id=' . $result['id'] . $url, 'SSL')
			);
            $this->load->model('catalog/attribute');
            $attInfo = $this->model_catalog_attribute->getAttribute($result['attribute_id']);
			$this->data['blocks'][] = array(
                'id'            => $result['id'],
				'attribute_id'  => $attInfo['name'],
		...
		$this->data['column_block_name'] = $this->language->get('column_block_name');
		$this->data['column_attribute_id'] = $this->language->get('column_attribute_id');
		...
		$this->data['sort_block_name'] = $this->url->link('customextension/block', 'token=' . $this->session->data['token'] . '&sort=tab1.block_name' . $url, 'SSL');
		$this->data['sort_attribute_id'] = $this->url->link('customextension/block', 'token=' . $this->session->data['token'] . '&sort=tab1.attribute_id' . $url, 'SSL');
		$url = '';
		...
		$pagination->url = $this->url->link('customextension/block', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

		$this->data['pagination'] = $pagination->render();

		$this->data['filter_block_name'] = $filter_block_name;
		$this->data['filter_attribute_id'] = $filter_attribute_id;

		$this->data['sort'] = $sort;
		$this->data['order'] = $order;

		$this->template = 'customextension/block_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['product_id'])) {
			$this->data['action'] = $this->url->link('customextension/block/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('customextension/block/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		}
		...
		$this->template = 'customextension/block_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
