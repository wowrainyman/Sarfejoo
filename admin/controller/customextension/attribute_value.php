<?php 
class ControllerCustomextensionAttributeValue extends Controller {
	private $error = array();

	public function index() {
		$this->language->load('customextension/attribute_value');

		$this->document->setTitle($this->language->get('heading_title')); 

		$this->load->model('customextension/pu_attribute_value');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            foreach ($_POST['value'] as $key => $value) {
                if($value!="")
                    $this->model_customextension_pu_attribute_value->addAttributeValue($id,$value);
            }
			...
			$this->redirect($this->url->link('customextension/attribute_value', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
        ...
        $this->data['insert'] = $this->url->link('customextension/attribute_value/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $attribute_total = $this->model_customextension_pu_attribute_value->getTotalAttributes();

        $results = $this->model_customextension_pu_attribute_value->getAttributes($data);
        ...
        foreach ($results as $result) {
            ...
            $action[] = array(
                ...
                'href' => $this->url->link('customextension/attribute_value/update', 'token=' . $this->session->data['token'] . '&attribute_id=' . $result['attribute_id'] . $url, 'SSL')
            );
        ...
        $this->data['sort_name'] = $this->url->link('customextension/attribute_value', 'token=' . $this->session->data['token'] . '&sort=tab3.name' . $url, 'SSL');
        $this->data['sort_attribute_group'] = $this->url->link('customextension/attribute_value', 'token=' . $this->session->data['token'] . '&sort=attribute_group' . $url, 'SSL');
        $this->data['sort_sort_order'] = $this->url->link('customextension/attribute_value', 'token=' . $this->session->data['token'] . '&sort=tab2.sort_order' . $url, 'SSL');
        ...
        $pagination->url = $this->url->link('customextension/attribute_value', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

        $this->data['form_action']=$this->url->link('customextension/attribute_value', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $this->template = 'customextension/attribute_value_list.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['product_id'])) {
			$this->data['action'] = $this->url->link('customextension/attribute_value/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('customextension/attribute_value/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		}
		...
		$this->template = 'customextension/attribute_value_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
