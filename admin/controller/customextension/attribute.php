<?php 
class ControllerCustomextensionAttribute extends Controller {
	private $error = array(); 

	public function index() {
		$this->language->load('customextension/attribute');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            foreach ($_POST['type'] as $key => $value) {
                ...
                $this->model_customextension_pu_attribute->UpdateAttributeType($key, $value);
            }
        }
		$this->getList();
	}
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$this->model_customextension_pu_attribute->addProductAttribute($id,$value);
			...
			$this->redirect($this->url->link('customextension/attribute', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$this->data['insert'] = $this->url->link('customextension/attribute/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$product_total = $this->model_customextension_pu_attribute->getTotalProducts($data);

		$results = $this->model_customextension_pu_attribute->getProducts($data);

        $this->data['form_action']=$this->url->link('customextension/attribute', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('customextension/attribute/update', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL')
			);
		...
		$this->data['sort_name'] = $this->url->link('customextension/attribute', 'token=' . $this->session->data['token'] . '&sort=tab3.name' . $url, 'SSL');
		$this->data['sort_model'] = $this->url->link('customextension/attribute', 'token=' . $this->session->data['token'] . '&sort=tab2.model' . $url, 'SSL');
        $this->data['sort_type'] = $this->url->link('customextension/attribute', 'token=' . $this->session->data['token'] . '&sort=tab1.type' . $url, 'SSL');
		$this->data['sort_order'] = $this->url->link('customextension/attribute', 'token=' . $this->session->data['token'] . '&sort=tab2.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('customextension/attribute', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'customextension/attribute_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['product_id'])) {
			$this->data['action'] = $this->url->link('customextension/attribute/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('customextension/attribute/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		}
		...
		$this->template = 'customextension/attribute_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
