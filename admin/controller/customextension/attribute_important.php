<?php 
class ControllerCustomextensionAttributeImportant extends Controller {
	private $error = array();

	public function index() {
		$this->language->load('customextension/attribute_important');

		$this->document->setTitle($this->language->get('heading_title')); 

		$this->load->model('customextension/pu_attribute_important');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            foreach ($_POST['value'] as $key => $value) {
                $this->model_customextension_pu_attribute_value->UpdateAttributeValue($key, $value);
            }
        }
		$this->getList();
	}
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $this->model_customextension_pu_attribute_important->addImportantAttribute($attribute_id,$product_id);
			...
			$this->redirect($this->url->link('customextension/attribute_important', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
        ...
        $this->data['insert'] = $this->url->link('customextension/attribute_important/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $attribute_important_total = $this->model_customextension_pu_attribute_important->getTotalImportantAttributes();

        $results = $this->model_customextension_pu_attribute_important->getImportantAttributes($data);
        ...
        foreach ($results as $result) {
            ...
            $action[] = array(
                ...
                'href' => $this->url->link('customextension/attribute_important/update', 'token=' . $this->session->data['token'] . '&attribute_id=' . $result['attribute_id'] . $url, 'SSL')
            );

            $this->data['important_attributes'][] = array(
                'id'              => $result['id'],
        ...
        $this->data['sort_id'] = $this->url->link('customextension/attribute_important', 'token=' . $this->session->data['token'] . '&sort=tab1.id' . $url, 'SSL');
        $this->data['sort_attribute_name'] = $this->url->link('customextension/attribute_important', 'token=' . $this->session->data['token'] . '&sort=tab2.name' . $url, 'SSL');
        $this->data['sort_product_name'] = $this->url->link('customextension/attribute_important', 'token=' . $this->session->data['token'] . '&sort=tab3.name' . $url, 'SSL');
        ...
        $pagination->total = $attribute_important_total;
        $pagination->page = $page;
        $pagination->limit = $this->config->get('config_admin_limit');
        $pagination->text = $this->language->get('text_pagination');
        $pagination->url = $this->url->link('customextension/attribute_important', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

        $this->data['form_action']=$this->url->link('customextension/attribute_important', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $this->template = 'customextension/attribute_important_list.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['product_id'])) {
			$this->data['action'] = $this->url->link('customextension/attribute_important/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('customextension/attribute_important/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		}
		...
		$this->template = 'customextension/attribute_important_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
