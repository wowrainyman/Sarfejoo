...
class ControllerCustomextensionPlanOnce extends Controller
{
    private $error = array();

    public function index()
    {
        $this->language->load('customextension/plan_once');

        $this->document->setTitle($this->language->get('heading_title'));

        $this->load->model('customextension/pu_plan_once');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            foreach ($_POST['name'] as $key => $value) {
                $this->model_customextension_pu_plan_once->UpdateName($key, $value);
            }
            foreach ($_POST['price'] as $key => $value) {
                $this->model_customextension_pu_plan_once->UpdatePrice($key, $value);
            }
            foreach ($_POST['available'] as $key => $value) {
                $this->model_customextension_pu_plan_once->UpdateAvailable($key, $value);
            }
            foreach ($_POST['target'] as $key => $value) {
                $this->model_customextension_pu_plan_once->UpdateTarget($key, $value);
            }
            foreach ($_POST['description'] as $key => $value) {
                $this->model_customextension_pu_plan_once->UpdateDescription($key, $value);
            }
            foreach ($_POST['sort_order'] as $key => $value) {
                $this->model_customextension_pu_plan_once->UpdateSortOrder($key, $value);
            }
        }
        $this->getList();
    }
    ...
    public function insert()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $this->model_customextension_pu_plan_once->addPlanonce(
                $name,
                $price,
                $available,
                $target,
            ...
            $this->redirect($this->url->link('customextension/plan_once', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
        $this->getForm();
    }
    ...
    protected function getList()
    {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('customextension/plan_once', 'token=' . $this->session->data['token'] . $url, 'SSL'),
            'separator' => ' :: '
        );

        $this->data['insert'] = $this->url->link('customextension/plan_once/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $plan_once_total = $this->model_customextension_pu_plan_once->getTotaloncePlans($data);

        $results = $this->model_customextension_pu_plan_once->getoncePlans($data);

        $this->data['form_action'] = $this->url->link('customextension/plan_once', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $this->data['sort_name'] = $this->url->link('customextension/plan_once', 'token=' . $this->session->data['token'] . '&sort=tab1.name' . $url, 'SSL');
        $this->data['sort_duration'] = $this->url->link('customextension/plan_once', 'token=' . $this->session->data['token'] . '&sort=tab1.duration' . $url, 'SSL');
        $this->data['sort_price'] = $this->url->link('customextension/plan_once', 'token=' . $this->session->data['token'] . '&sort=tab1.price' . $url, 'SSL');
        $this->data['sort_available'] = $this->url->link('customextension/plan_once', 'token=' . $this->session->data['token'] . '&sort=tab1.available' . $url, 'SSL');
        $this->data['sort_target'] = $this->url->link('customextension/plan_once', 'token=' . $this->session->data['token'] . '&sort=tab1.target' . $url, 'SSL');
        ...
        $pagination->total = $plan_once_total;
        $pagination->page = $page;
        $pagination->limit = $this->config->get('config_admin_limit');
        $pagination->text = $this->language->get('text_pagination');
        $pagination->url = $this->url->link('customextension/plan_once', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->template = 'customextension/plan_once_list.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    protected function getForm()
    {
        ...
        if (!isset($this->request->get['product_id'])) {
            $this->data['action'] = $this->url->link('customextension/plan_once/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $this->data['action'] = $this->url->link('customextension/plan_once/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
        }
        ...
        $this->template = 'customextension/plan_once_form.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
