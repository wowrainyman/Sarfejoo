...
class ControllerCustomextensionPayment extends Controller
{
    private $error = array();

    public function index()
    {
        $this->language->load('customextension/payment');

        $this->document->setTitle($this->language->get('heading_title'));

        $this->load->model('customextension/pu_payment');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            foreach ($_POST['subject'] as $key => $value) {
                $this->model_customextension_pu_payment->UpdateSubject($key, $value);
            }
            foreach ($_POST['ison'] as $key => $value) {
                $this->model_customextension_pu_payment->updateIson($key, $value);
            }
            foreach ($_POST['cost'] as $key => $value) {
                $this->model_customextension_pu_payment->updateCost($key, $value);
            }
        }
        $this->getList();
    }
    ...
    public function insert()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $this->model_customextension_pu_payment->addPayment($subject, $ison, $cost);
            ...
            $this->redirect($this->url->link('customextension/payment', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
    ...
    protected function getList()
    {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('customextension/payment', 'token=' . $this->session->data['token'] . $url, 'SSL'),
            'separator' => ' :: '
        );

        $this->data['insert'] = $this->url->link('customextension/payment/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $payment_total = $this->model_customextension_pu_payment->getTotalPayments($data);

        $results = $this->model_customextension_pu_payment->getPayments($data);

        $this->data['form_action'] = $this->url->link('customextension/payment', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $this->data['column_is_on'] = $this->language->get('column_is_on');
        ...
        $this->data['sort_subject'] = $this->url->link('customextension/payment', 'token=' . $this->session->data['token'] . '&sort=tab1.subject' . $url, 'SSL');
        $this->data['sort_cost'] = $this->url->link('customextension/payment', 'token=' . $this->session->data['token'] . '&sort=tab1.cost' . $url, 'SSL');
        $this->data['sort_is_on'] = $this->url->link('customextension/payment', 'token=' . $this->session->data['token'] . '&sort=tab1.is_on' . $url, 'SSL');
        ...
        $pagination->url = $this->url->link('customextension/payment', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->template = 'customextension/payment_list.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    protected function getForm()
    {
        ...
        $this->data['entry_is_on'] = $this->language->get('entry_is_on');
        ...
        if (!isset($this->request->get['product_id'])) {
            $this->data['action'] = $this->url->link('customextension/payment/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $this->data['action'] = $this->url->link('customextension/payment/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
        }
        ...
        $this->template = 'customextension/payment_form.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
