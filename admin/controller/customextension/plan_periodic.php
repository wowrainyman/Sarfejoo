...
class ControllerCustomextensionPlanPeriodic extends Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        $this->load->model('customextension/pu_plan_periodic');
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            foreach ($_POST['name'] as $key => $value) {
                $this->model_customextension_pu_plan_periodic->UpdateName($key, $value);
            }
            foreach ($_POST['duration'] as $key => $value) {
                $this->model_customextension_pu_plan_periodic->UpdateDuration($key, $value);
            }
            foreach ($_POST['price'] as $key => $value) {
                $this->model_customextension_pu_plan_periodic->UpdatePrice($key, $value);
            }
            foreach ($_POST['available'] as $key => $value) {
                $this->model_customextension_pu_plan_periodic->UpdateAvailable($key, $value);
            }
            foreach ($_POST['target'] as $key => $value) {
                $this->model_customextension_pu_plan_periodic->UpdateTarget($key, $value);
            }
            foreach ($_POST['description'] as $key => $value) {
                $this->model_customextension_pu_plan_periodic->UpdateDescription($key, $value);
            }
            foreach ($_POST['sort_order'] as $key => $value) {
                $this->model_customextension_pu_plan_periodic->UpdateSortOrder($key, $value);
            }
        }
        $this->getList();
    }
    ...
    public function insert()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $available = $this->request->post['available'];
            $target = $this->request->post['target'];
            $description = $this->request->post['description'];
            $sort_order = $this->request->post['sort_order'];
            $this->model_customextension_pu_plan_periodic->addPlanPeriodic(
                $name,
                $duration,
                $price,
                $available,
            ...
            if (isset($this->request->get['filter_target'])) {
                $url .= '&filter_target=' . urlencode(html_entity_decode($this->request->get['filter_target'], ENT_QUOTES, 'UTF-8'));
            }
            ...
            $this->redirect($this->url->link('customextension/plan_periodic', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
        $this->getForm();
    }
    ...
    protected function getList()
    {
        ...
        if (isset($this->request->get['filter_target'])) {
            $filter_target = $this->request->get['filter_target'];
        } else {
            $filter_target = null;
        }
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('customextension/plan_periodic', 'token=' . $this->session->data['token'] . $url, 'SSL'),
            'separator' => ' :: '
        );

        $this->data['insert'] = $this->url->link('customextension/plan_periodic/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $plan_periodic_total = $this->model_customextension_pu_plan_periodic->getTotalPeriodicPlans($data);

        $results = $this->model_customextension_pu_plan_periodic->getPeriodicPlans($data);

        $this->data['form_action'] = $this->url->link('customextension/plan_periodic', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        foreach ($results as $result) {
            ...
            $action[] = array(
                ...
                'href' => $this->url->link('customextension/payment/update', 'token=' . $this->session->data['token'] . '&product_id=' . $url, 'SSL')
            );

            $this->data['plan_periodics'][] = array(
                'id' => $result['id'],
                'name' => $result['name'],
        ...
        $this->data['column_available'] = $this->language->get('column_available');
        ...
        $this->data['sort_name'] = $this->url->link('customextension/plan_periodic', 'token=' . $this->session->data['token'] . '&sort=tab1.name' . $url, 'SSL');
        $this->data['sort_duration'] = $this->url->link('customextension/plan_periodic', 'token=' . $this->session->data['token'] . '&sort=tab1.duration' . $url, 'SSL');
        $this->data['sort_price'] = $this->url->link('customextension/plan_periodic', 'token=' . $this->session->data['token'] . '&sort=tab1.price' . $url, 'SSL');
        $this->data['sort_available'] = $this->url->link('customextension/plan_periodic', 'token=' . $this->session->data['token'] . '&sort=tab1.available' . $url, 'SSL');
        $this->data['sort_target'] = $this->url->link('customextension/plan_periodic', 'token=' . $this->session->data['token'] . '&sort=tab1.target' . $url, 'SSL');
        ...
        $pagination->total = $plan_periodic_total;
        $pagination->page = $page;
        $pagination->limit = $this->config->get('config_admin_limit');
        $pagination->text = $this->language->get('text_pagination');
        $pagination->url = $this->url->link('customextension/plan_periodic', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

        $this->data['pagination'] = $pagination->render();

        $this->data['filter_name'] = $filter_name;
        $this->data['filter_target'] = $filter_target;

        $this->data['sort'] = $sort;
        $this->data['order'] = $order;

        $this->template = 'customextension/plan_periodic_list.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    protected function getForm()
    {
        ...
        if (!isset($this->request->get['product_id'])) {
            $this->data['action'] = $this->url->link('customextension/plan_periodic/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $this->data['action'] = $this->url->link('customextension/plan_periodic/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
        }

        $this->data['cancel'] = $this->url->link('customextension/attribute', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $this->template = 'customextension/plan_periodic_form.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
