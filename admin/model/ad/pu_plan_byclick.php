...
class ModelAdPuPlanByclick extends Model {
    public function getTotalByclickPlans($data = array()){
        $pu_database_name = $GLOBALS['pu_database_name'];
        $oc_database_name = $GLOBALS['oc_database_name'];

        $sql = "SELECT COUNT(DISTINCT tab1.id) AS total FROM $pu_database_name.pu_ad_byclick_plan tab1 WHERE 1";
    ...
    public function getByclickPlans($data){
        $pu_database_name = $GLOBALS['pu_database_name'];
        $oc_database_name = $GLOBALS['oc_database_name'];

        $sql = "SELECT * FROM $pu_database_name.pu_ad_byclick_plan tab1 WHERE 1";
    ...
    public function UpdateSortOrder($id,$value) {
        ...
        $exist = false;
        $sql_select_string = "SELECT * FROM `pu_ad_byclick_plan` WHERE `id` = $id";
        ...
        if ($exist) {
            $sql_update_string = "UPDATE `pu_ad_byclick_plan` SET " .
                "`sort_order`='$value'" .
                " WHERE `id` = $id";
    ...
    public function UpdateClicks($id,$value) {
        $con_PU_db = $GLOBALS['con_PU_db'];
        $exist = false;
        $sql_select_string = "SELECT * FROM `pu_ad_byclick_plan` WHERE `id` = $id";
        $result_select = mysqli_query($con_PU_db, $sql_select_string) or die(mysqli_error());
    ...
    public function addPlanByclick(
        $name,
        $enable,
        $sort_order,
        $price,
        $clicks)
    {
        $pu_database_name = $GLOBALS['pu_database_name'];
        $con_PU_db = $GLOBALS['con_PU_db'];
        $sql_insert_string = "INSERT INTO $pu_database_name.pu_ad_byclick_plan" .
            "(`name`, `enable`, `sort_order`, `price`, `clicks`)" .
            " VALUES ('$name', '$enable', '$sort_order', '$price', '$clicks');";
