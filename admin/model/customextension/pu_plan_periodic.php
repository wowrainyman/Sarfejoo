...
class ModelCustomextensionPuPlanPeriodic extends Model {
    public function getTotalPeriodicPlans($data = array()){
        $pu_database_name = $GLOBALS['pu_database_name'];
        $oc_database_name = $GLOBALS['oc_database_name'];

        $sql = "SELECT COUNT(DISTINCT tab1.id) AS total FROM $pu_database_name.pu_plan_periodic tab1 WHERE 1";

        if (!empty($data['filter_name'])) {
            $sql .= " AND tab1.name LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
        }
        if (!empty($data['filter_target'])) {
            $sql .= " AND tab1.target LIKE '%" . $this->db->escape($data['filter_target']) . "%'";
        }
    ...
    public function getPeriodicPlans($data){
        $pu_database_name = $GLOBALS['pu_database_name'];
        $oc_database_name = $GLOBALS['oc_database_name'];

        $sql = "SELECT * FROM $pu_database_name.pu_plan_periodic tab1 WHERE 1";
    ...
    public function UpdateSortOrder($id,$value) {
        ...
        if ($exist) {
            $sql_update_string = "UPDATE `pu_plan_periodic` SET " .
                "`sort_order`='$value'" .
                " WHERE `id` = $id";
    ...
    public function UpdateTarget($id,$value) {
        $con_PU_db = $GLOBALS['con_PU_db'];
        $exist = false;
        $sql_select_string = "SELECT * FROM `pu_plan_periodic` WHERE `id` = $id";
        $result_select = mysqli_query($con_PU_db, $sql_select_string) or die(mysqli_error());
    ...
    public function addPlanPeriodic(
        $name,
        $duration,
        $price,
        $available,
        ...
        $sort_order)
    {
        $pu_database_name = $GLOBALS['pu_database_name'];
        $con_PU_db = $GLOBALS['con_PU_db'];
        $sql_insert_string = "INSERT INTO $pu_database_name.pu_plan_periodic" .
            "(`name`, `duration`, `price`, `available`, `target`, `description`, `sort_order`)" .
            " VALUES ('$name', '$duration', '$price', '$available', '$target', '$description', '$sort_order');";
