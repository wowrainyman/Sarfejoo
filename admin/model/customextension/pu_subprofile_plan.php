...
class ModelCustomextensionPuSubprofilePlan extends Model {
    ...
    public function getPlans($data){
        ...
        $sql = "SELECT * FROM $pu_database_name.pu_subprofile_plans tab1 WHERE 1";
    ...
    public function UpdateIsProduct($id,$value) {
        $con_PU_db = $GLOBALS['con_PU_db'];
        $exist = false;
        $sql_select_string = "SELECT * FROM `pu_subprofile_plans` WHERE `id` = $id";
        $result_select = mysqli_query($con_PU_db, $sql_select_string) or die(mysqli_error());
        ...
        $is_product)
    {
        ...
        $sql_insert_string = "INSERT INTO $pu_database_name.pu_subprofile_plans" .
            "(`name`, `duration`, `price`, `active`, `sort_order`, `is_product`)" .
            " VALUES ('$name', '$duration', '$price', '$active', '$sort_order', '$is_product');";
