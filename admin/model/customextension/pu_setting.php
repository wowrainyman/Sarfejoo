...
class ModelCustomextensionPuSetting extends Model {
    public function getTotalSettings($data = array()){
        ...
        $sql = "SELECT COUNT(DISTINCT id) AS total FROM $pu_database_name.pu_setting tab1";
    ...
    public function getSettings($data){
        $pu_database_name = $GLOBALS['pu_database_name'];
        $oc_database_name = $GLOBALS['oc_database_name'];
        $sql = "SELECT * FROM $pu_database_name.pu_setting tab1";
    ...
    public function UpdateKey($id,$value) {
        ...
        $exist = false;
        $sql_select_string = "SELECT * FROM `pu_setting` WHERE `id` = $id";
        ...
        if ($exist) {
            $sql_update_string = "UPDATE `pu_setting` SET " .
                "`key`='$value'" .
                " WHERE `id` = $id";
    ...
    public function addSetting($key,$value) {
        $con_PU_db = $GLOBALS['con_PU_db'];
        $sql_insert_string = "INSERT INTO `pu_setting`" .
            "(`key`, `value`)" .
            "VALUES ('$key', '$value');";
