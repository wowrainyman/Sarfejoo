...
class ControllerModulepumenu extends Controller {
	...
	public function index() {   
		$this->load->language('module/pumenu');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('pumenu', $this->request->post);		
 		...
 		if (isset($this->error['code'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/pumenu', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/pumenu', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['pumenu_module'])) {
			$this->data['modules'] = $this->request->post['pumenu_module'];
		} elseif ($this->config->get('pumenu_module')) { 
			$this->data['modules'] = $this->config->get('pumenu_module');
		}			
		...
		$this->template = 'module/pumenu.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/pumenu')) {
