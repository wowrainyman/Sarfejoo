...
var GMapsLatLonPicker = (function() {
	...
	var performSearch = function(string, silent) {
		if (string == "") {
			if (!silent) {
			// ---M	displayError( _self.params.strings.error_empty_field );
			}
			return;
		}
		_self.vars.geocoder.geocode(
			...
			function(results, status) {
				if (status == google.maps.GeocoderStatus.OK) {
					...
					if (!silent) {
					// ---M	displayError( _self.params.strings.error_no_results );
					}
				}
			}
		);
	...
	var publicfunc = {
		...
		init : function(object) {
			...
			// Search function by search change
			$(_self.vars.cssID + "#address").bind("change paste keyup", function() {
