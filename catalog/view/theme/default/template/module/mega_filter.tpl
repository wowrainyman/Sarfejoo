		...
		<ul>
			<?php foreach( $filters as $kfilter => $filter ) { ?>
				...
				<li
					...
					<div class="mfilter-content-opts"<?php echo ( $filter['collapsed'] == '1' || ( $is_mobile && $filter['collapsed'] == 'mobile' ) || ( ! $is_mobile && $filter['collapsed'] == 'pc' ) ) && $filter['collapsed'] != 'hide_header' && $_position != 'content_top' ? ' style="display:none"' : ''; ?>>
