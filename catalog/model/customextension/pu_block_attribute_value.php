...
class ModelCustomextensionPuBlockAttributeValue extends Model
{
    ...
    public function getSubattributeValues($subattribute_id){
        $pu_database_name = $GLOBALS['pu_database_name'];
        $con_PU_db = $GLOBALS['con_PU_db'];
        $sql_select_string = "SELECT * FROM $pu_database_name.pu_block_attribute_value WHERE `block_attribute_id` = $subattribute_id";
        $result_select = mysqli_query($con_PU_db, $sql_select_string) or die(mysqli_error());
        $result = array();
        while ($row = mysqli_fetch_assoc($result_select)) {
            $result[] = $row;
