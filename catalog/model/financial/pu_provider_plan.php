...
class ModelFinancialPuProviderPlan extends Model {
    public function getTotalPlans($data = array()){
        ...
        $sql = "SELECT COUNT(DISTINCT tab1.id) AS total FROM $pu_database_name.pu_provider_plans tab1 WHERE 1";
    ...
    public function getPlans(){
        $pu_database_name = $GLOBALS['pu_database_name'];
        $oc_database_name = $GLOBALS['oc_database_name'];
        $sql = "SELECT * FROM $pu_database_name.pu_provider_plans tab1 WHERE active = 1";
    ...
    public function getPlan($id){
        ...
        $pu_database_name = $GLOBALS['pu_database_name'];
        $oc_database_name = $GLOBALS['oc_database_name'];
        $sql = "SELECT * FROM $pu_database_name.pu_provider_plans tab1 WHERE active = 1 AND id = $id";
    ...
    public function UpdateSortOrder($id,$value) {
        $con_PU_db = $GLOBALS['con_PU_db'];
        $exist = false;
        $sql_select_string = "SELECT * FROM `pu_provider_plans` WHERE `id` = $id";
        ...
        if ($exist) {
            $sql_update_string = "UPDATE `pu_provider_plans` SET " .
                "`sort_order`='$value'" .
                " WHERE `id` = $id";
        ...
        $sort_order)
    {
        $pu_database_name = $GLOBALS['pu_database_name'];
        $con_PU_db = $GLOBALS['con_PU_db'];
        $sql_insert_string = "INSERT INTO $pu_database_name.pu_provider_plans" .
